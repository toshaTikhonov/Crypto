...
/**
   @brief RSA private-key decryption, according to PKCS #1.
 */
...
/** @brief Raw RSA private-key operation. Output has same length as modulus.
...
/**
   @brief Raw RSA public-key operation. Output has same length as modulus.
       Assumes inputLen < length of modulus.
       Requires input < modulus.
 */
int RSAPublicBlock (
...
/**
  \brief RSA RAW public-key decryption
 */
...
)
{
  ...
  if ((status = RSAPublicBlock
      (output, outputLen, input, inputLen, publicKey)) != 0)
    return (status);
...
/**
   \brief RSA RAW private-key encryption
 */
...
)
{
  ...
  //if ((status = RSAPrivateBlock (output, outputLen, input, inputLen, privateKey)) != 0)
    
  publicKey.bits = privateKey->bits;
  memcpy (publicKey.modulus, privateKey->modulus, sizeof (publicKey.modulus));
  memcpy (publicKey.exponent, privateKey->exponent, sizeof (publicKey.exponent));
  if ((status = RSAPublicBlock (output, outputLen, input, inputLen, &publicKey)) != 0)
